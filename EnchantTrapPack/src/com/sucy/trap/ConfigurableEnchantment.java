...
public class ConfigurableEnchantment extends CustomEnchantment {
    ...
    public ConfigurableEnchantment(Plugin plugin, EnchantDefaults enchant, Material[] items, int weight, ConflictGroup group) {
        ...
        suffixGroups.add(SuffixGroups.TRAP.getKey());
    ...
    /**
     * Calculates the number of entities to spawn
     *
     * @param level enchantment level
     * @return      number of entities to spawn
     */
    public int spawn(int level) {
        return getInt(ConfigValues.SPAWN_BASE.getKey()) + getInt(ConfigValues.SPAWN_BONUS.getKey()) * (level - 1);
    }
